...
regd_users.put("/auth/review/:isbn", (req, res) => {
  ...
  if (book.reviews[req.session.authorization.username]) {
    ...
    return res.status(208).json({ message: "Review updated successfully!" });
  } else {
    book.reviews[req.session.authorization.username] = review;
    return res.status(208).json({ message: "Review added successfully!" });
  }
});
...
regd_users.delete("/auth/review/:isbn", (req, res) => {
  ...
  if (book.reviews[req.session.authorization.username]) {
    ...
    return res.status(208).json({ message: "Review deleted successfully!" });
  } else {
